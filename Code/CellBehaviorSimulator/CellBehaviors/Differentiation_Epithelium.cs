...
{
    public partial class Differentiation_Epithelium : UserControl
    {
        public Differentiation_Epithelium()
        {
            InitializeComponent();
        }

        #region EventHandler
        private void Differentiation_Epithelium_Load(object sender, EventArgs e)
        {
        ...
        private void Differentiation_Epithelium_VisibleChanged(object sender, EventArgs e)
        {
            if (!Visible)
            {
        ...
        #endregion
        public const string Str_C_T_dif = "C_T_dif&Epithelium";
        ...
        public static bool ReadParameter(string strName, int typeNum, string strValue)
        {
            switch (strName)
            {
                ...
                case "C_T_dif":
                    Differentiation_Basal_AsyncDivision.SetParameter_C_T_dif(typeNum, strValue);
                    Differentiation_Basal_Decay.SetParameter_C_T_dif(typeNum, strValue);
                    return SetParameter_C_T_dif(typeNum, strValue);
        ...
        public static void Run2(List<CellData> cells, int[] ind)
        {
            Parallel.For(0, ind.Length, i =>
            {
                Run2(cells, ind[i]);
            });
        }
        public static void Run2(List<CellData> cells, int index)
        {
